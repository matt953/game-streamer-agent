//! Send-side bandwidth estimation from per-packet feedback (spec 04, ABR v2):
//! delay-gradient control (trendline + AIMD), loss-based control, a probing
//! state machine with padding clusters, ALR detection with periodic probing,
//! and link-capacity memory. Third-party notice: bwe/THIRD-PARTY.md.

pub(crate) mod average;
pub(crate) mod bandwidth;
pub(crate) mod time_tricks;
pub(crate) mod types;

/// Local names for the vendored modules' shared imports.
pub(crate) mod prelude {
    pub(crate) use super::average::MovingAverage;
    pub(crate) use super::bandwidth::{Bitrate, DataSize};
    pub(crate) use super::time_tricks::{already_happened, not_happening};
    #[cfg(test)]
    pub(crate) use super::types::TwccPacketId;
    pub(crate) use super::types::{Reason, Soonest, TwccClusterId, TwccSendRecord, TwccSeq};
}
...
use crate::bwe::prelude::Reason;
use crate::bwe::prelude::Soonest;
use crate::bwe::prelude::{Bitrate, DataSize, TwccClusterId, TwccSendRecord, TwccSeq};
...
pub(crate) use probe::ProbeControl;
