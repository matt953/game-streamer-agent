//! Probing controller: decides when and how to burst padding at rates above
//! the current estimate to discover available capacity.

use std::collections::VecDeque;
use std::time::{Duration, Instant};

use super::{ProbeClusterConfig, ProbeKind};
use crate::bwe::prelude::{Bitrate, TwccClusterId};
use crate::bwe::prelude::{already_happened, not_happening};
...
impl ProbeControl {
    ...
    fn compute_next_timeout(&mut self, now: Instant) -> Instant {
        ...
        if let Some(last) = &self.last_probe
            && matches!(last.kind, ProbeKind::Initial | ProbeKind::Exponential)
        {
