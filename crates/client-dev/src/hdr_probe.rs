...
// The bitstream-parsing half of this probe (SEI walking, HDR payload
// extraction) lives in `gsa_client_core::hdr`, shared with the FFI so the
// apps read the same bytes the same way. Re-exported here for the decoder.
pub use gsa_client_core::hdr::{
    HdrPayloads, StaticMetadata, av1_static_metadata, hevc_hdr_payloads,
};
